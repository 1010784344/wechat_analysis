...
def sex_ratio(fri_infos):
    ...
    man_ratio = float(man)/total * 100
    woman_ratio = float(woman)/total * 100
    other_ratio = float(other)/total * 100
    plt.rcParams['font.sans-serif'] = ['SimHei']    # 用来正常显示中文标签
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    plt.figure(figsize=(5, 5))  # 绘制的图片为正圆
    sex_li = [u'男', u'女', u'其他']
    radius = [0.01, 0.01, 0.02]  # 设定各项距离圆心n个半径
