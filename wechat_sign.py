...
def word_cloud(comment):
    ...
    comment_txt = ''
    back_img = plt.imread(conf_path + u'/peiqi.jpg')
    cloud = WordCloud(font_path=conf_path + u'/simhei.ttf',  # 若是有中文的话，这句代码必须添加，不然会出现方框，不出现汉字
    ...
    wc.to_file(res_full + u'好友个性签名词云图.png')
    # logger.info('end %s ' % func_name)
...
def snowlp_analysis(comment):
    ...
    plt.savefig(res_full + u'好友签名情感分析')
    plt.show()
    # logger.info('end %s ' % func_name)
